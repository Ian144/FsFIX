...
let WriteUnderlyingStipulations (strm:System.IO.Stream) (grp:UnderlyingStipulations) =
    grp.NoUnderlyingStipsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoUnderlyingStipsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteUnderlyingInstrument (strm:System.IO.Stream) (grp:UnderlyingInstrument) =
    ...
    grp.NoUnderlyingSecurityAltIDGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoUnderlyingSecurityAltIDGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteNoNestedPartyIDsGrp (strm:System.IO.Stream) (grp:NoNestedPartyIDsGrp) =
    ...
    grp.NoNestedPartySubIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoNestedPartySubIDsGrp strm))    // group WRITE THE NOGROUP FIELD


// component
let WriteNestedParties (strm:System.IO.Stream) (grp:NestedParties) =
    grp.NoNestedPartyIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoNestedPartyIDsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteNoNested2PartyIDsGrp (strm:System.IO.Stream) (grp:NoNested2PartyIDsGrp) =
    ...
    grp.NoNested2PartySubIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoNested2PartySubIDsGrp strm))    // group WRITE THE NOGROUP FIELD


// component
let WriteNestedParties2 (strm:System.IO.Stream) (grp:NestedParties2) =
    grp.NoNested2PartyIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoNested2PartyIDsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteInstrumentLeg (strm:System.IO.Stream) (grp:InstrumentLeg) =
    ...
    grp.NoLegSecurityAltIDGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoLegSecurityAltIDGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteLegStipulations (strm:System.IO.Stream) (grp:LegStipulations) =
    grp.NoLegStipulationsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoLegStipulationsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteNoPartyIDsGrp (strm:System.IO.Stream) (grp:NoPartyIDsGrp) =
    ...
    grp.NoPartySubIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoPartySubIDsGrp strm))    // group WRITE THE NOGROUP FIELD


// component
let WriteParties (strm:System.IO.Stream) (grp:Parties) =
    grp.NoPartyIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoPartyIDsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteStipulations (strm:System.IO.Stream) (grp:Stipulations) =
    grp.NoStipulationsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoStipulationsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteTradeCaptureReport_NoSidesGrp (strm:System.IO.Stream) (grp:TradeCaptureReport_NoSidesGrp) =
    ...
    grp.NoClearingInstructionsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoClearingInstructionsGrp strm))    // group WRITE THE NOGROUP FIELD
    ...
    grp.NoContAmtsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoContAmtsGrp strm))    // group WRITE THE NOGROUP FIELD
    grp.Stipulations |> Option.iter (WriteStipulations strm) // component
    grp.NoMiscFeesGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoMiscFeesGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteNoSettlPartyIDsGrp (strm:System.IO.Stream) (grp:NoSettlPartyIDsGrp) =
    ...
    grp.NoSettlPartySubIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoSettlPartySubIDsGrp strm))    // group WRITE THE NOGROUP FIELD


// component
let WriteSettlParties (strm:System.IO.Stream) (grp:SettlParties) =
    grp.NoSettlPartyIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoSettlPartyIDsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteSettlInstructionsData (strm:System.IO.Stream) (grp:SettlInstructionsData) =
    ...
    grp.NoDlvyInstGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoDlvyInstGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteInstrument (strm:System.IO.Stream) (grp:Instrument) =
    ...
    grp.NoSecurityAltIDGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoSecurityAltIDGrp strm))    // group WRITE THE NOGROUP FIELD
    grp.Product |> Option.iter (WriteProduct strm)
    grp.CFICode |> Option.iter (WriteCFICode strm)
    grp.SecurityType |> Option.iter (WriteSecurityType strm)
    ...
    grp.NoEventsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoEventsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteNewOrderList_NoOrdersGrp (strm:System.IO.Stream) (grp:NewOrderList_NoOrdersGrp) =
    ...
    grp.AllocID |> Option.iter (WriteAllocID strm)
    grp.PreallocMethod |> Option.iter (WritePreallocMethod strm)
    grp.NoAllocsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoAllocsGrp strm))    // group WRITE THE NOGROUP FIELD
    ...
    grp.NoTradingSessionsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoTradingSessionsGrp strm))    // group WRITE THE NOGROUP FIELD
    grp.ProcessCode |> Option.iter (WriteProcessCode strm)
    WriteInstrument strm grp.Instrument    // component
...
let WriteMultilegOrderCancelReplaceRequest_NoLegsGrp (strm:System.IO.Stream) (grp:MultilegOrderCancelReplaceRequest_NoLegsGrp) =
    ...
    grp.LegSwapType |> Option.iter (WriteLegSwapType strm)
    grp.LegStipulations |> Option.iter (WriteLegStipulations strm) // component
    grp.NoLegAllocsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoLegAllocsGrp strm))    // group WRITE THE NOGROUP FIELD
    grp.LegPositionEffect |> Option.iter (WriteLegPositionEffect strm)
    grp.LegCoveredOrUncovered |> Option.iter (WriteLegCoveredOrUncovered strm)
    grp.NestedParties |> Option.iter (WriteNestedParties strm) // component
    grp.LegRefID |> Option.iter (WriteLegRefID strm)
...
let WriteNoNested3PartyIDsGrp (strm:System.IO.Stream) (grp:NoNested3PartyIDsGrp) =
    ...
    grp.NoNested3PartySubIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoNested3PartySubIDsGrp strm))    // group WRITE THE NOGROUP FIELD


// component
let WriteNestedParties3 (strm:System.IO.Stream) (grp:NestedParties3) =
    grp.NoNested3PartyIDsGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoNested3PartyIDsGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteInstrumentExtension (strm:System.IO.Stream) (grp:InstrumentExtension) =
    ...
    grp.NoInstrAttribGrp |> Option.iter (fun xs -> xs |> List.iter (WriteNoInstrAttribGrp strm))    // group WRITE THE NOGROUP FIELD
...
let WriteMassQuoteAcknowledgement_NoQuoteSetsGrp (strm:System.IO.Stream) (grp:MassQuoteAcknowledgement_NoQuoteSetsGrp) =
    ...
    grp.MassQuoteAcknowledgement_NoQuoteEntriesGrp |> Option.iter (fun xs -> xs |> List.iter (WriteMassQuoteAcknowledgement_NoQuoteEntriesGrp strm))    // group WRITE THE NOGROUP FIELD
...
// group
let WriteNetworkStatusResponse_NoCompIDsGrp (strm:System.IO.Stream) (grp:NetworkStatusResponse_NoCompIDsGrp) =
    grp.RefCompID |> Option.iter (WriteRefCompID strm)
    grp.RefSubID |> Option.iter (WriteRefSubID strm)
    grp.LocationID |> Option.iter (WriteLocationID strm)
    grp.DeskID |> Option.iter (WriteDeskID strm)
