...
let ReadSingleCaseUTCTimeOnlyField  (pos:int) (bs:byte[]) fldCtor =
    ...
    let tm = UTCDateTimex.readUTCTimeOnly bs pos pos2
...
let ReadSingleCaseUTCDateField  (pos:int) (bs:byte[]) fldCtor =
    let pos2 = FIXBufUtils.findNextFieldTermOrEnd pos bs
    let dt = UTCDateTimex.readUTCDate bs pos pos2
    pos2 + 1,  fldCtor dt

let ReadSingleCaseUTCTimestampField (pos:int) (bs:byte[]) fldCtor =
    let pos2 = FIXBufUtils.findNextFieldTermOrEnd pos bs
    let dt = UTCDateTimex.readUTCTimestamp bs pos pos2
    pos2 + 1,  fldCtor dt
...
let inline WriteFieldUTCTimeOnly
    ...
    Buffer.BlockCopy (tag, 0, dest, nextFreeIdx, tag.Length)
    let nextFreeIdx2 = nextFreeIdx + tag.Length
    let nextFreeIdx3 =  UTCDateTimex.writeUTCTimeOnly tm dest nextFreeIdx2
    dest.[nextFreeIdx3] <- 1uy // write the SOH field delimeter
    nextFreeIdx3 + 1 // +1 to move past the delimeter
...
let inline WriteFieldUTCDate
    ...
    Buffer.BlockCopy (tag, 0, dest, nextFreeIdx, tag.Length)
    let nextFreeIdx2 = nextFreeIdx + tag.Length
    let nextFreeIdx3 =  UTCDateTimex.writeUTCDate tm dest nextFreeIdx2
    dest.[nextFreeIdx3] <- 1uy // write the SOH field delimeter
    nextFreeIdx3 + 1 // +1 to move past the delimeter
...
let inline WriteFieldUTCTimestamp
    ...
    Buffer.BlockCopy (tag, 0, dest, nextFreeIdx, tag.Length)
    let nextFreeIdx2 = nextFreeIdx + tag.Length
    let nextFreeIdx3 =  UTCDateTimex.writeUTCTimestamp tm dest nextFreeIdx2
    dest.[nextFreeIdx3] <- 1uy // write the SOH field delimeter
    nextFreeIdx3 + 1 // +1 to move past the delimeter
