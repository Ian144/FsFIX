...
let ``write valid hhmmss to bytes`` () =
    ...
    let posOut = FIXDateTime.writeBytesUTCTimestamp timeIn bs 0
...
let ``read valid hhmmss from bytes`` () =
    let uto = FIXDateTime.fromBytesUTCTimestamp "20161213-23:59:59"B 0 17
...
let ``read valid hhmmss midnight from bytes`` () =
    let uto = FIXDateTime.fromBytesUTCTimestamp "20161213-00:00:00"B 0 17
...
let ``read valid leapsecond hhmmss from bytes`` () =
    let uto = FIXDateTime.fromBytesUTCTimestamp "20161213-23:59:60"B 0 17
...
let ``read valid leapsecond hhmmssMMM from bytes`` () =
    ...
    let uto = FIXDateTime.fromBytesUTCTimestamp bs  0 bs.Length
