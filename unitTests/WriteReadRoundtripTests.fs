...
// this test fails with quickfixN builds of FsFix because the bytes are encoded with quickfixJ's version of MassQuote, which has UnderlyingRepoCollateralSecurityType as a string not an int
//[<Fact>]
...
//    let index = Array.zeroCreate<FIXBufIndexer.FieldPos> (1024 * 8)
